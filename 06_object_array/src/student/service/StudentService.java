...
public class StudentService {
	...
	public StudentService() {
		...
		students[1] = new StudentDTO("24002", "철수", '남');
		students[2] = new StudentDTO("24003", "이슬", '여');
		// students[3] / [4] == null
		...
		for(StudentDTO std : students) { // 향상된 for문
			if(std == null) break; // 현재 접근한 요소가 참조하는 객체가 없다면 반복을 종료한다.
	 ...
	 * - 실패 조건 : students 배열에 꽉 차있는데 추가 할생이 전달된 경우
	 */
	public boolean addStudent(StudentDTO std) {
		
		// 0번 인덱스부터 순서대로 비어있는(null)인 요소를 찾아 전달 받은 std를 대입
		...
		// 비어있는(null)인 요소가 없을 경우 false를 반환
		return false;
	}
	/**
	...
	public StudentDTO selectName(String targetName) {
		...
		for(StudentDTO std : students) {
			// 저장된 학생이 없을 경우 -> 뒤에도 없을 것 -> 그냥 여기서 검색 종료
	...
	/**
	 * 전달 받은 index가 students 범위 내 인지 + 정상 범위라면 해당 index가 학생 객체를 참조하는지 확인
	 * @param index
	 * @return 1 또는 2 또는 3
	 */
	...
	public void updateScores(int index, StudentDTO other) {
		...
		// students[index] 번 째에 저장된 주소 얕은 복사
