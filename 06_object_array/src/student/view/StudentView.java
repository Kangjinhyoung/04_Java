...
public class StudentView {
	...
	private void allStudent() {
		...
		for(StudentDTO std : students) { // 향상된 for문
			...
			// -> NullPointerException 방지 (참조변수가 null을 저장하고 있는데 사용하려고 할 때)
			if(std == null) return;
	...
	/**
	 * 인덱스를 입력받아 service에 전달하여 해당 인덱스의 학생 객체 하나(StudentDTO)를 반환 받아 출력
	 */
	private void selectIndex() {
		...
		if(std == null) {
			System.out.println("*** 해당 인덱스에 학생 객체가 존재하지 않아요 ***");
			return;
		}
	...
	public void selectName() {
		...
		// 입력 받은 이름(targetName)을 service로 전달하여 일치하는 이름을 가진 학생 객체(주소) 반환 받기
		// -> null 또는 학생 객체 (주소 값)
		StudentDTO std = service.selectName(targetName);
		if(std == null) {
			System.out.println("*** 같은 이름의 학생이 존재하지 않아요 ***");
			return;
		}
		System.out.println(std.toString());
	}
	/**
	 * 인덱스 번호를 입력 받아 일치하는 학생의 html, css, js, java 점수를 수정하기
	 * 단, 입력된 인덱스가 0 미만 students 배열 마지막 인덱스 초과한 경우 
	 * "인덱스 범위가 올바르지 않습니다" (1)
	 * 정상 범위 인덱스이지만 학생이 존재하지 않는 경우 "해당 인덱스에 학생 정보가 존재하지 않습니다" (2)
	 * 정상 (3)
	 */
	private void updateIndex() {
		...
		System.out.print("HTML, CSS, JS, Java 순서로 점수를 입력 : ");
	...
	private void testMaxMin() {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// 최소값 비교
			if(arr[i] < min) min = arr[i]; // min에 새 최소값 대입
		}
