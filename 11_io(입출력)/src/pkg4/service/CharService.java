...
public class CharService {
	...
	public void fileOutput1() {
		...
		try { // 예외가 발생할 것 같은 코드를 작성하는 구문
			...
			// 문자열을 지정된 파일에 출력 -> 자동으로 전달한 String을 한 글자씩 출력
			fw.write(str);
	...
	/**
	 * 문자 기반 입력 스트림을 이용해
	 * /io_test/char/문자테스트.txt 파일 내용을 읽어와 콘솔에 출력
	 */
	public void fileInput1() {
		...
		try {
			...
			while(true) {
				...
				// fr.read() : 문자 하나로 읽어옴, 없으면 -1
				value = fr.read();
				
				if(value == -1) break; // 더 이상 읽어올 문자가 없을 경우
	...
	public void fileInput2() {
		...
		try {
			...
			while(true) {
				...
				if(line == null) break; // 읽어온 내용이 없다면 반복 종료
