...
public class ThrowsService {
	 ...
	 *       돌려 받는 최종 결과가 정살 결과인지, 예외 처리된 결과인지 구분하기 힘들어지는 경우가 발생
	 */
	...
	public void test2() throws Exception {
		// (2) throws Exception
		// -> 메서드 내부에서 발생한 Exception 또는 Exception 자식 
		//    예외를 메서드 호출한 곳으로 다시 던진다
	...
	public void test5() {
		...
		do {
			...
			try {
				...
				default : System.out.println("*** 잘못 입력했어요 ㅋ ***");
				}
				...
				input = -1; // 첫 반복 시 반복문이 종료 되지 않도록 input값을 변경
	...
	public void divNum() /* throws RuntimeException */{
		...
		// num2가 0인 경우 ArithmeticException(산술적 예외) 발생 가능성이 있다!
		// -> 빨간줄 안뜨는 이유 : Unchecked Exception이라서 빨간줄 안뜸
		
		// ArithmeticException 발생하면 호출한 곳으로 예외를 던지는 thorws 사용!
		
		// ***Unchecked Exception은 아무런 예외처리가 안되어있는 
		//    경우 컴파일러가 자동으로 throws 구문을 생성해준다!! ***
	}
	...
	public void inputKeyboard() throws IOException{
		...
		// -> 입력된 키보드 값을 문자로 변경하여 버퍼에 모은 후 한방에 읽어오는 객체를 생성함
		
		System.out.print("문자열 입력 : ");
		String str = br.readLine();
		
		// br.readLine() : 입력된 한 줄을 통째로 읽어옴
		// -> readLine() 메서드는 IOException을 던질 가능성이 
		//    있으므로 예외 처리 구문을 "꼭" 작성해 예외 상황에 대비
