...
public class TryCatchFinallyService {
	 ...
	 * 
	 * finally {} : try구문에서 예외 발생 여부 상관 없이 무조건 
	 *              마지막에 실행되어야 하는 코드를 작성하는 구문
	 */
	...
	public void test2() {
		...
		//    scan이 참조하는 객체가 닫히지 않아서 자원(메모리)이 누수되고 있어
		//    --> 키보드와 연결된 통로(Stream)가 닫히지 않고 남아있어서 메모리가 소비됨
		...
		try {
			...
			// 예외 발생 여부 관계없이 무조건 외부 입력 장치와의 
			// 통로를 닫아서 메모리를 반환 시켜주는 메서드
		}
