...
public class Parent extends Object{
	 ...
	 *  
	 * - Object.toString() 역할
	 *   -> 물려 받은 자식이 오버라이딩 하여 객체가 가지고 있는 
	 *      필드값을 하나의 문자열로 만드는 것임(약속 되어 있음)
	 *   -> print() 관련 구문에서 ()안에 참조 변수명만 
	 *      작성해도 자동으로 toString()이 호출된다.
	 */
	@Override // 오버라이딩 잘 되있는지 검사
	public String toString() {
		return String.format("[Parent] %s / %s / %d / %s", lastName, address, money, car);
	}
	/* final 메서드
	 * - 오버라이딩 불가 -> 부모의 메서드가 완벽해서 더 이상 오버라이딩을 못하게 함(마지막으로 정의된 메서드)
