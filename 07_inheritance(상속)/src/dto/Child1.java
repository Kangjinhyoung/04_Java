...
public class Child1 extends Parent{
	 ...
	 * - super()는 자식 생성자에만 작성할 수 있으며 반드서 첫 번째 줄에 작성해야 한다.
	 */
...
//		return String.format("%s / %s / %d / %s / %s", lastName, getAddress(), getMoney(), getCar(), notebook);
	 ...
	 * Over   : 위에 있다, 넘어서다 
	 * Riding : 타는 것
	 * - 부모로 부터 물려 받은 메서드를 자식이 같은 이름의 메서드를 만들어서 덮어쓰기
	 *   -> 물려 받은 자식의 맞게 부모의 메서드를 다시 정의해야 되는 경우 사용 
	 *   
	 * ** 중요 **
	 * @Override 어노테이션(Annotation)
	 * 1) 해당 메서드는 오버라이딩 되었음을 명시
	 * 2) 오버라이딩된 코드가 알맞게 작성 되었는지 컴파일러에게 검사하라고 지시하는 컴파일러 주석
	 * 
	 * [오버라이딩 성립 조건]
	 * 1. 접근 제한자는 같거나 더 넓은 범위만 가능
	 * 2. 반환형, 매개 변수, 메서드명은 모두 동일해야 한다.
	 * 3. 예외처리 구문(throws)은 같거나 좁은 범위
	 * (쉽게 말해 부모랑 똑같이 작성하는게 제일 쉬움)
	 */
	
	@Override // 컴파일러에게 이거 잘 쓴건지 검사시키는 것
	...
	public String toString() {
		...
		// Child1 클래스 내부에서 soString() 호출하면 Child1.toString()이 호출됨.
		
		// super.toString() : 부모 클래스에 작성된 toString() 호출
		// -> 오버라이딩 했다고 해서 부모의 메서드가 덮어 쓰기되어 사라지는 것이 아니다!
