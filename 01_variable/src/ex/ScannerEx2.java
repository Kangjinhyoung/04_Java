...
public class ScannerEx2 {
	public static void main(String[] args) {
		...
		/* Scanner는 char를 제외한 기본 자료형 7가지를 
		 * 입력받는 기능(메서드)을 제공한다
		 * + 문자열 입력 방법 2개
		 *   ( next() , nextLine() )
		 * 
		 ...
		 * - nextLine()을 제외한 next 관련 구문들은 입력 버퍼에 
		 * 	 저장된 값 중 해당 자료형 값을 앞에서 부터 읽어옴
		 *   단, 제일 앞에 [엔터] 있으면 "제거" 후 그 다음 부터 읽음
		 * - nextLine() :
		 ...
		 * 
		 * - nextInt() 입력 후 버퍼에 [엔터]가 남아 있어서
		 * 	 다음 nextLine()이 남아있는 [엔터]를 읽어 오고 동작 종료
		 * 
		 *  -- 해결 방법
		 *  -> 중간에 nextLine()구문을 추가하여 남아있는 [엔터]를 꺼내가서 없앰
		 *  
		 *  int num = sc.nextInt();
