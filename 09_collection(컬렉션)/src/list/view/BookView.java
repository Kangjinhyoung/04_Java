...
public class BookView {
	...
	/**
	 * 전체 조회
	 * - BookService에 있는 bookList를 얻어와 존재하는 모든 요소의 정보를 출력한다.
	 */
	private void selectAll() {
		System.out.println("\n----- 전체 조회 -----\n");
		List<BookDTO> bookList = service.selectAll();
		for(int i = 0; i < bookList.size(); i++) {
			// print() 관련 구문에서 참조 변수명을 작성하면 자동으로 toString() 호출된다.
	...
	private void selectIndex() {
		...
		// 전달한 index 번째 요소가 존재하면 BookDTO 객체 주소 없다면 null(참조하는 객체가 없음) 반환
	...
	/**
	 * 검색할 가격의 범위(최소값, 최대값)을 입력받아 가격 범위 내에 모든 책을 조회하여 출력하기
	 * 단, 없으면 "조회 결과가 없습니다" 출력
	 */
	...
	/**
	 * 제목, 글쓴이, 가격을 입력받아 BookService의 BookList에 추가 하기
	 */
	...
	private void removeBook() {
		...
		System.out.print("제거할 책의 index 번호 입력 : ");
		int index = sc.nextInt();
	...
	/**
	 * index 번호를 입력 받아 해당 index에 책이 존재하지 않으면 
	 * -> "해당 인덱스에 책이 존재하지 않습니다" 출력 후 메서드 종료
	 * 해당 index에 책이 존재하면
	 * -> 수정할 가격을 입력 후 수정 -> "[책 제목] 가격이 (이전 가격) -> (새 가격) 수정 되었습니다" 출력
	 */
	private void updateBook() {
		System.out.println("\n----- 책 가격 수정하기 -----\n");
		System.out.print("수정할 책의 index 번호 입력 : ");
		int index = sc.nextInt();
		
		// 인덱스가 일치하는 요소의 BookDTO(참조 주소) 반환 받기
