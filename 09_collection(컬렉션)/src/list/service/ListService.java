 ...
 * 
 * [ List ]
 * - 자료(Data)를 순차적으로 나열한 자료 구조(배열과 비슷하다)
 * - index가 존재함
 *   -> 각 Data를 index(순서)로 구분할 수 있어 중복된 데이터를 저장할 수 있다.
 * 
 */

public class ListService {
	public void test1() {
		// ArrayList 객체 생성 -> List 인터페이스를 상속 받은 자식 클래스로 가장 많이 사용하는 List이다.
...
//		List list = new ArrayList(); // 10칸 짜리 생성
		...
		// 4번째 -> 크기 초과 오류 발생 X -> List 크기가 자동으로 증가
		list.add(123); // int 기본 자료형은 추가 불가 -> Integer Wrapper class로 변환되어 저장된다.
	...
	/*
	 * List + 제네릭(타입제한) 확인 + List 메서드 몇가지 더
	 */
	public void test2() {
		/* 제네릭(Generics, <>)
		...
		// add() 확인
		// menuList.add(123); // Stirng으로 제한되서 다른 자료형 추가 불가
		menuList.add("짬뽕");
		menuList.add("보쌈");
		menuList.add("닭갈비");
		menuList.add("양념갈비");
		...
		// - 지정된 index번째 요소를 e(두 번째 매개 변수)로 수정 -> 메서드 반환 값으로 이전 객체가 반환됨  
		System.out.println("-----------------------------------------------------------------");
		...
		// - List의 내부에 e와 같은 객체가 존재하면 true, 없으면 false (확인 기능)
		System.out.println("보쌈 : " + menuList.contains("보쌈"));
		System.out.println("막국수 : " + menuList.contains("막국수"));
		
		// 7. int indexOf(E e)
		// - List의 내부에 e와 같은 객체가 존재하면 해당 index를 반환 존재하지 않으면 -1 반환
		System.out.println("삼겹살 : " + menuList.indexOf("삼겹살")); // 4
		System.out.println("오겹살 : " + menuList.indexOf("오겹살")); // -1
		
		// 8. E remove(int index)
		// - List에서 지정된 index번째 요소를 제거 -> 중간이 제거되면 뒤쪽 요소를 앞으로 이동됨
		// - 제거된 요소는 메서드 결과로 반환됨
		System.out.println(menuList.remove(2) + " 제거");
		System.out.println(menuList);	
	}
	/**
	 * 왜 컬렉션 참조 변수를 부모타입으로 작성하는가?
	 * - 프로그래밍 과정에서 미쳐 생각지 못한 비효율적인 요소를 쉽게 개선하기 위해서
	 *   ex) ArrayList로 만들었는데 중간에 추가, 삭제가 빈번히 일어나는 경우 -> LinkedList가 훨씬 효율적이다
	 *   	 LinkedList로 만들었는데 검색이 빈번하게 일어나는 경우 -> ArrayList가 훨씬 효율적
	 */
	public void test3() {
...
//		List<Integer> list = new ArrayList<Integer>();
		...
		// -> 1970년 1월 1일 09:00:00.00 기준으로 현재 시간 까지 지난 시간을 ms로 반환(long) 
