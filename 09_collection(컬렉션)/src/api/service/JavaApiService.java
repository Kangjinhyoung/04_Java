...
public class JavaApiService {
	 ...
	 *       -> 참조하는 객체의 주소가 달라진다! (다른 객체가 된다는 뜻)
	 */
	 ...
	 *   -> 모든 객체가 다른 결과를 반환
	 *   
	 * - 같은 객체의 저장된 값이 변했다면 주소값은 일정해야만 한다!
	 * 
	 ...
	 * 
	 * - 동일한 문자열을 생성하게 되는 경우 객체가 새롭게 생성되지 않고 기존 주소가 반환된다(재활용)
	 */
	public void test2() {
		
		String s1 = "Hello"; // 1554547125 (상수풀 생성)
		System.out.println("s1 : " + System.identityHashCode(s1));
		
		// 같은 값을 지닌 String 생성
		String s2 = "Hello"; // 1554547125 (상수풀 객체 재활용)
		System.out.println("s2 : " + System.identityHashCode(s2));
		
		// 같은 값을 지닌 String을 new 연산자를 이용해 생성
		String s3 = new String("Hello"); // 951007336 (Heap에 새로 생성)
		System.out.println("s3 : " + System.identityHashCode(s3));
		
		System.out.println("-------------------------------------------");
		
		// s1, s2는 같은 객체를 참조하고 있음 == s1, s2에 저장된 주소값이 같음
		// -> equals()를 사용하지 않아도 비교 가능
		
		System.out.println("s1 == s2 : " + (s1 == s2) ); // true
		
		// s1, s3는 저장된 값은 같아도 서로 다른 객체를 찹조하고 있음 
		// == s1, s3의 주소값이 다르기 때문에 비교 연산 시 무조건 false 
	...
	public void test3() {
		...
		/* 문자열 추가 전 주소 */
		System.out.println(System.identityHashCode(sb)); // 1554547125
		...
		/* 문자열 추가 후 주소 */
		// -> 주소가 계속 똑같음! 객체의 필드값만 변하고 있음(가변성 확인)
		System.out.println(System.identityHashCode(sb)); // 1554547125
	...
	public void test4() {
		...
		// - String 배열을 하나의 문자열로 변환, 각 요소 사이에 "구분자" 추가
		...
		System.out.println(str2); // Hi World				
	}	
}
