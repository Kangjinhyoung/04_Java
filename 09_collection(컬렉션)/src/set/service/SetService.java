...
public class SetService {
	...
	public void test1() {
		...
		// 3. boolean remove(E e) : Set에 저장된 객체 중 같은 객체를 찾아 제거 제거 성공 : true / 없으면 : false
		System.out.println("쿠팡 제거 : " + set.remove("쿠팡"));
		System.out.println("삼성 제거 : " + set.remove("삼성"));
		System.out.println(set);
		
		// 4. boolean contains(E e) : 같은 요소가 있으면 : true / 없으면 : false
		System.out.println("네이버 있음? " + set.contains("네이버"));
		System.out.println("KT 있음? " + set.contains("KT"));
		
		// 5. boolean isEmpty() : 비어 있으면 : true / 아니면 false
		// 6. void clear() : 저장된 객체를 모두 제거
		System.out.println("비어있음? " + set.isEmpty()); // 5
		...
		// Set 
		// - index가 없음 -> index를 활용하는 메서드가 없다 -> 원하는 요소 하나만을 꺼낼 수 없다.
		// - 순서 유지 X -> 중복 허용 X
	}
	/**
	 * Set에 저장된 객체를 꺼내는 방법
	...
	public void test2() {
		...
		/* 1. Iterator(반복자) 이용 */
		// - 컬렉션에 저장된 요소를 하나 씩 순차접근하는 기능을 제공하는 인터페이스	
		Iterator<String> it = snackSet.iterator();
		
		System.out.println("[Iterator]");
		while(it.hasNext()) { // hasNext() : 다음 순차 접근할 요소가 있으면 true
			String snack = it.next(); // next() : 다음 요소가 있으면 꺼내와서 반환 -> 화살표를 한 칸 움직임
	 ...
	 * ex) HastSet, LinkedHastSet, HashMap, HashTable 등
	 * 1. Object.equals() 오버라이딩 하기
	 * 2. Object.hashCode() 오버라이딩 하기
	 * *** 1, 2 둘 다 수행 해야만 함
	 * -> Java는 동일한 객체임을 비교할 때 동등(equals) / 동일(hashCode) 
	 *    모두 true여야지만 동일한 객체로 인식한다.
	 */
	public void test3() {
		...
		// 확인 1(equals, hashCode 오버라이딩 안한 상태) -> 유리가 중복되서 출력
		for(Person p : set) System.out.println(p);
		
		System.out.println("-----------------------------------------");
		
		// equals 오바라이딩 확인 == 동등 비교(동명이인 개념)
	...
	/** 로또 번호 생성기
	 * - 금액을 천원 단위로 입력 받아 천원당 1회씩 번호를 생성해서 
	 *   List에 저장한 후 생성 종료되는 시점에 한 번에 출력
	 *   단, 5회 출력 시 마다 구분선 추가
	 * <pre>
	 * 금액 입력 : 6000
	 * 
