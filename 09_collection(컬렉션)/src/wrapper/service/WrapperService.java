...
public class WrapperService {
	...
	public void test1() {
		...
		// (컴파일러가 new Interger(num1) 형태로 변환)
		
		/* Integer -> int로 Unboxing */
		int num2 = wrap1.intValue(); // 정석 방법
		int num3 = wrap1; // (Integer)
		// -> 컴파일러가 자동으로 Auto Unboxing 수행 (wrap1 -> wrap1.intValue()로 변환)
...
//		List<int> list = new ArrayList<int>(); -> 제네릭에 기본 자료형 작성 시 오류 발생
		...
		// 향상된 for문 - 좌변 : 각 인덱스(요소) 값을 얻어와 저장할 변수, 우변 : 배열 또는 컬렉션
	...
	/**
	 * Wrapper Class가 제공하는 필드, 메서드 -> 대부분이 static으로 되어 있음!
	 * -> 프로그램 실행 시 영역에 필드 / 메서드 생성 --> 클래스명.메서드명() 사용 가능
	 */
	public void test2() {
		// 정수형 / 실수형 공통 필드
		...
		// 이를 Java에서 기본 자료형으로 변환 하는 경우 사용!! 단, char(Character)는 존재하지 않음!
